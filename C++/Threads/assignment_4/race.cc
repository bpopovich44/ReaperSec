...
pthread_mutex_t skywalker = PTHREAD_MUTEX_INITIALIZER;

void *foo (void *bar)
{
	pthread_t *me = new pthread_t (pthread_self());
  
	//Added mutex lock because of shared resource
  	pthread_mutex_lock(&skywalker);
	...
	pthread_mutex_unlock(&skywalker);
