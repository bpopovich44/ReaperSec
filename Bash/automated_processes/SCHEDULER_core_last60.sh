...
##
#
# Script runs and checks core_last60 database tables accross all platforms
#
##
...
file5="oath_core_last60.py"

script="core_last60.py"
table="core_last60"
...
core_last60(){
	...
	# Error handling -- LEAVING RETURN ON EXIT CODE 0 FOR NOW, IF THERE IS AN ERROR OTHER PLATFORMS WILL RUN.  Error log will still be updated
	check_db=$(${mysql_db}"SELECT NULL FROM "${platform}_${table}" WHERE DATE IN ("\'${TODAYDATE_1}\'", "\'${TODAYDATE_2}\'") LIMIT 1")
...
launch_processes(){
	...
	do
		[[ "$?" == "0" ]] && core_last60 ${platform} || return 1
	done
...
#run_core_last60 > >(tee -a ${path_hourly_log}) 2> >(tee -a ${path_error_log} >&2) 
