...
##
#
# Script runs and checks core_yesterday_media database tables accross all platforms
#
##
...
file5="oath_core_yesterday_media.py"

script="core_yesterday_media.py"
table="core_yesterday_media"
...
launch_processes(){
	...
	do
		[[ "$?" == "0" ]] && core_yesterday_media ${platform} & 
	done
...
#run_core_yesterday_media > >(tee -a ${path_hourly_log}) 2> >(tee -a ${path_error_log} >&2) 
