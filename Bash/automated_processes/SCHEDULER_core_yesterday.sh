...
##
#
# Script runs and checks core_yesterday database tables accross all platforms
#
##
...
file5="oath_core_yesterday.py"

script="core_yesterday.py"
table="core_yesterday"
...
launch_processes(){
	...
	do
		[[ "$?" == "0" ]] && core_yesterday ${platform} || return 1
	done
...
#run_core_yesterday > >(tee -a ${path_hourly_log}) 2> >(tee -a ${path_error_log} >&2) 
