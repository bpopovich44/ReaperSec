...
run_inventorySources(){
	...
	remainder=$(($invsources_v2_count%$num_per_rotation))
        
	rownum_start=0
        rownum_end=299999

	echo inventorysources_count $invsources_v2_count
		...
		echo rownum_start_$rownum_start
		echo rownum_end_$rownum_end

        done

        echo ${path_pan}-file="${path_ktr}${platform}_inventorysources_v2.ktr" -param:ROWNUM_START="${rownum_end}" -param:ROWNUM_END="${invsources_v2_count}"
...
run_domain(){
	...
	echo ${path_pan}-file="${path_ktr}${prefix}_domain_v2.ktr" -param:PROCESS_REPORT="${platform}_${report}_v2"

	[[ "$?" == '0' ]] && run_inventorySources ${platform} ${report} ${prefix} || return 1
	}
...
daily_domain(){
	...
	#LOOK AT daily_process.log for more information." >> ${path_error_log}; return 1; } || return 0 #run_domain ${platform} ${report} ${prefix}
...
launch_processes(){
	...
	do
		[[ "$?" == "0" ]] && for report in "${report_book[@]}" 
		do
			[[ "$?" == "0" ]] && daily_domain ${report} ${platform}  
		done
	done 
...
run_daily_domain(){
	...
	## STEP 3 run_domain and piggyback to run_inventorySource
#        [[ "$?" == '0' ]] && run_domain || return 1
	 
	## STEP 4
#	[[ "$?" == '0' ]] && send_reports

	## STEP 7 Entry into Epiphany_logs		
#	log_entry
	}
