...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS dc_market_private_EOM"
            cursor.execute(sql)

            sql = "CREATE TABLE dc_market_private_EOM (date varchar(25), inventory_source varchar(255), geo_country varchar(50), \
            ...
            result = aol_api.run_existing_report(logintoken, "193893")
            #print(result)
	        ...
	        sql = """INSERT INTO dc_market_private_EOM VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", \
