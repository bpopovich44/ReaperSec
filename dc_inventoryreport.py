...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS dc_inventoryreport"
            cursor.execute(sql)

            sql = "CREATE TABLE dc_inventoryreport (date varchar(50), inventory_source varchar(255), geo_country varchar(50), \
		...
		ad_revenue decimal(15, 5), ecpm decimal(15, 5), platform int)"
            ...
            result = aol_api.run_existing_report(logintoken, "143128")
            #print(result)
...
#	        print(list)
        	...
        	sql = """INSERT INTO dc_inventoryreport VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s")""" % \
