...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS adtrans_market_private"
            cursor.execute(sql)

            sql = "CREATE TABLE adtrans_market_private (date varchar(25), inventory_source varchar(255), geo_country varchar(50), \
            ...
            result = aol_api.run_existing_report(logintoken, "169699")
            #print(result)
	        ...
	        sql = """INSERT INTO adtrans_market_private VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", \
			"%s", "%s")""" % \
