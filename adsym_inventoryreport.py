...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS adsym_inventoryreport"
            cursor.execute(sql)

            sql = "CREATE TABLE adsym_inventoryreport (date varchar(50), inventory_source varchar(255), geo_country varchar(50), \
		...
		ad_revenue decimal(15, 5), ecpm decimal(15, 5), platform int)"
            ...
            result = aol_api_R.run_existing_report(logintoken, "161172")
            #print(result)
...
#	        print(list)
        	...
        	sql = """INSERT INTO adsym_inventoryreport VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s")""" % \
