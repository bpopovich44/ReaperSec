...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS dna_market_public_EOM"
            cursor.execute(sql)

            sql = "CREATE TABLE dna_market_public_EOM (date varchar(25), inventory_source varchar(255), geo_country varchar(50), \
            ...
            result = aol_api.run_existing_report(logintoken, "193890")
            #print(result)
	        ...
	        sql = """INSERT INTO dna_market_public_EOM VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", \
