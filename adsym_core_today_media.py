...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS adsym_core_today_media"
            cursor.execute(sql)
    
            sql = "CREATE TABLE adsym_core_today_media (inventory_source varchar(255), media varchar(255), ad_opportunities bigint, \
            ...
            result = aol_api_R.run_existing_report(logintoken, "161188")
            #print(result)
...
#	        print(list)
                ...
                sql = """INSERT INTO adsym_core_today_media VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s"*.20, \
