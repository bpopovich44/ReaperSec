...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS adtrans_core_last60"
            cursor.execute(sql)

            sql = "CREATE TABLE adtrans_core_last60 (date varchar(50), inventory_source varchar(255), ad_opportunities bigint, \
            ...
            result = aol_api_R.run_existing_report(logintoken, "169869")
            #print(result)
...
#                print(list)
	        ...
	        sql = """INSERT INTO adtrans_core_last60 VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s"*.20, "%s"*.67, \
