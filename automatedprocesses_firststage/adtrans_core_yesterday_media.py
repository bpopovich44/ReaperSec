...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS adtrans_core_yesterday_media"
            cursor.execute(sql)

            sql = "CREATE TABLE adtrans_core_yesterday_media (date varchar(25), hour int, inventory_source varchar(255), media varchar(255),  \
            ...
            result = aol_api_R.run_existing_report(logintoken, "169871")
            #print(result)
        	...
        	sql = """INSERT INTO adtrans_core_yesterday_media VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", \
			"%s"*.20, "%s"*.67, "%s"*.24, "%s", "%s", "%s", "%s")""" % (date, hour, inventory_source, media,
