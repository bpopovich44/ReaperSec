...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS v3_core_last60"
            cursor.execute(sql)

            sql = "CREATE TABLE v3_core_last60 (date varchar(50), inventory_source varchar(255), ad_opportunities bigint, \
            ...
            result = aol_api_R.run_existing_report(logintoken, "194308")
            #print(result)
	        ...
	        sql = """INSERT INTO v3_core_last60 VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", \
		    	"%s", "%s")""" % (date, inventory_source, ad_opportunities, market_opportunities, ad_attempts, \
                        ad_impressions, ad_errors, ad_revenue, total_clicks, iab_viewability_measurable_ad_impressions, iab_viewable_ad_impressions)
                cursor.execute(sql)
