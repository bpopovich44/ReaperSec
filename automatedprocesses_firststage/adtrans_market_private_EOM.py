...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS adtrans_market_private_EOM"
            cursor.execute(sql)

            sql = "CREATE TABLE adtrans_market_private_EOM (date varchar(25), inventory_source varchar(255), geo_country varchar(50), \
            ...
            logintoken = aol_api.get_access_token("629065c0-a967-473b-b62e-cf4353d9f5c7", "3GLSSYb1BxWdlN3Iu8/p7A")
            print(logintoken)

            result = aol_api.run_existing_report(logintoken, "193904")
            #print(result)
	        ...
	        sql = """INSERT INTO adtrans_market_private_EOM VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", \
