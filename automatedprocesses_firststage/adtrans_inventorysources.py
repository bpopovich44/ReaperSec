...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS  adtrans_inventorysources"
            cursor.execute(sql)

            sql = "CREATE TABLE  adtrans_inventorysources (date varchar(25), inventory_source varchar(255), ad_opportunities bigint, \
		ad_attempts bigint, ad_impressions bigint, ad_revenue decimal(15, 5), ecpm decimal(15, 5), platform int)"
            ...
            result = aol_api_R.run_existing_report(logintoken, "169696")
            #print(result)
	        ...
	        sql = """INSERT INTO  adtrans_inventorysources VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s")""" % \
