...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            # Create Table:
            sql = "DROP TABLE IF EXISTS adtrans_core_today"
            cursor.execute(sql)

            sql = "CREATE TABLE adtrans_core_today (hour int, inventory_source varchar(255), ad_opportunities bigint, \
            ...
            result = aol_api_R.run_existing_report(logintoken, "169872")
            #print(result)
        	...
        	sql = """INSERT INTO adtrans_core_today VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s"*.20, "%s"*.67, \
