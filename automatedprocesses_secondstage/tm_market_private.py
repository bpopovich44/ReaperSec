...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS tm_market_private"
            cursor.execute(sql)

            sql = "CREATE TABLE tm_market_private (date varchar(25), inventory_source varchar(255), geo_country varchar(50), \
            ...
            result = aol_api.run_existing_report(logintoken, "169371")
            #print(result)
	        ...
	        sql = """INSERT INTO tm_market_private VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", \
			"%s", "%s")""" % \
