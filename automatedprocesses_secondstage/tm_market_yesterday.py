...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS tm_market_yesterday"
            cursor.execute(sql)

            sql = "CREATE TABLE tm_market_yesterday (date varchar(25), hour int, buyer_organization varchar(255), \
		    ...
		    tm_revenue decimal(15, 5), total_clicks int, iab_viewability_measurable_ad_impressions bigint, \
            ...
            result = aol_api.run_existing_report(logintoken, "169841")
            #print(result)
	        ...
	        sql = """INSERT INTO tm_market_yesterday VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s"*.20, \
			"%s"*.64, "%s"*.24, "%s", "%s", "%s", "%s")""" % (date, hour, buyer_organization, inventory_source, \
