...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS dc_core_yesterday"
            cursor.execute(sql)

            sql = "CREATE TABLE dc_core_yesterday (date varchar(25), hour int, inventory_source varchar(255), ad_opportunities bigint, \
		    market_opportunities bigint, ad_attempts bigint, ad_impressions bigint, ad_errors bigint, ad_revenue decimal(15, 5), \
	    	    aol_cost decimal(15, 5), epiphany_gross_revenue decimal(15, 5), dc_revenue decimal(15, 5), \
            ...
            result = aol_api.run_existing_report(logintoken, "143984")
            #print(result)
...
#                print(list)
        	...
        	sql = """INSERT INTO dc_core_yesterday VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s"*.20, \
