...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS  dna_inventorysources"
            cursor.execute(sql)

            sql = "CREATE TABLE  dna_inventorysources (date varchar(25), inventory_source varchar(255), ad_opportunities bigint, \
		ad_attempts bigint, ad_impressions bigint, ad_revenue decimal(15, 5), ecpm decimal(15, 5), platform int)"
            ...
            result = aol_api.run_existing_report(logintoken, "102622")
            #print(result)
	        ...
	        sql = """INSERT INTO  dna_inventorysources VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s")""" % \
