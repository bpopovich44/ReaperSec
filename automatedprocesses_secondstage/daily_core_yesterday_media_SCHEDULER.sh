...
##
#
# Script runs and checks daily_core_last60 database tables
#
##
...
send_email=${path_kitchen}-file="${path_kjb}daily_core_yesterday_media_ERROR.kjb"
...
core_yesterday_media(){
	...
	${path_sbin}"${platform}_core_yesterday_media.py"
	check_db=$(${mysql_db}"SELECT NULL FROM "${platform}_core_yesterday_media" LIMIT 1")
...
run_core_yesterday_media(){

	for platform in "${platforms[@]}"
	do
		core_yesterday_media ${platform} &
	done
