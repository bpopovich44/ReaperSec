...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS tm_market_today"
            cursor.execute(sql)

            sql = "CREATE TABLE tm_market_today (hour int, buyer_organization varchar(255), inventory_source varchar(255),  \
            ...
            result = aol_api.run_existing_report(logintoken, "169840")
            #print(result)
	        ...
	        list = (hour, buyer_organization, inventory_source, market_opportunities, ad_attempts, ad_impressions, \
                ...
                sql = """INSERT INTO tm_market_today VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", \
