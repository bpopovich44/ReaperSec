...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS dc_market_last60"
            cursor.execute(sql)

            sql = "CREATE TABLE dc_market_last60 (date varchar(50), buyer_organization varchar(255), inventory_source varchar(255), \
            ...
            result = aol_api.run_existing_report(logintoken, "150662")
            #print(result)
	        ...
	        sql = """INSERT INTO dc_market_last60 VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s"*.20, "%s"*.56, \
