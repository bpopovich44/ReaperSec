...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS dna_market_private"
            cursor.execute(sql)

            sql = "CREATE TABLE dna_market_private (date varchar(25), inventory_source varchar(255), geo_country varchar(50), \
            ...
            result = aol_api.run_existing_report(logintoken, "193232")
            #print(result)
	        ...
	        sql = """INSERT INTO dna_market_private VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", \
			"%s", "%s")""" % \
