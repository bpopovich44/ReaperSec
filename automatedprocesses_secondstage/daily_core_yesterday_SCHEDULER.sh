...
##
#
# Script runs and checks daily_core_yesterday tables
#
##
...
send_email=${path_kitchen}-file="${path_kjb}daily_core_yesterday_ERROR.kjb"
...
core_yesterday(){
	...
	${path_sbin}"${platform}_core_yesterday.py"
	check_db=$(${mysql_db}"SELECT NULL FROM "${platform}_core_yesterday" LIMIT 1")
