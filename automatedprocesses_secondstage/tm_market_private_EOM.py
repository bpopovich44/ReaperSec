...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS tm_market_private_EOM"
            cursor.execute(sql)

            sql = "CREATE TABLE tm_market_private_EOM (date varchar(25), inventory_source varchar(255), geo_country varchar(50), \
            ...
            result = aol_api.run_existing_report(logintoken, "193901")
            #print(result)
	        ...
	        sql = """INSERT INTO tm_market_private_EOM VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", \
