...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS tm_core_last60"
            cursor.execute(sql)

            sql = "CREATE TABLE tm_core_last60 (date varchar(50), inventory_source varchar(255), ad_opportunities bigint, \
            ...
            result = aol_api.run_existing_report(logintoken, "169832")
            #print(result)
...
#                print(list)
	        ...
	        sql = """INSERT INTO tm_core_last60 VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s"*.20, "%s"*.64, \
