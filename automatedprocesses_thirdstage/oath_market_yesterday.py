            ...
            for report in report_book[report_type][p_name]:
                ...
                if int(row_count_value) >= 1:
                    if db_updated == False:
                        ...
                        sql = "DROP TABLE IF EXISTS " + p_name + "_market_yesterday"
                        cursor.execute(sql)

                        sql = "CREATE TABLE " + p_name + "_market_yesterday (date varchar(25), hour int, buyer_organization varchar(255), \
                ...
                print(str(todaysdate) + "  Running " + p_name + "_market_yesterday id_ " + str(p_id) + " report # " + str(report))
	            ...
	            #print(list)
                    ...
                    sql = """INSERT INTO """ + p_name + """_market_yesterday VALUES ("%s", "%s", "%s", "%s", "%s", "%s", \
                        "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s")""" % (date, hour, buyer_organization, \
                        inventory_source, market_opportunities, ad_attempts, ad_impressions, ad_errors, ad_revenue, aol_cost, \
