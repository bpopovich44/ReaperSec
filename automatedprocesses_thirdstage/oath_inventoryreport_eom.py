...
def connect():
    ...
    db = "mysql_eom"
    report_type = "inventoryreport_eom"
    p_name = sys.argv[1]
            ...
            for report in report_book[report_type][p_name]: 
                ...
                if len(result) >= 1:
                    if db_updated == False:
                        ...
                        sql = "DROP TABLE IF EXISTS " + p_name + "_inventoryreport_EOM"
                        cursor.execute(sql)

                        sql = "CREATE TABLE " + p_name + "_inventoryreport_EOM (date varchar(50), inventory_source varchar(255), \
                ...
                print(str(todaysdate) + "  Running " + p_name + "_inventoryreport_EOM id_" + str(p_id) + " with report # " + str(report))
        	    ...
        	    sql = """INSERT INTO """ + p_name + """_inventoryreport_EOM VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", \
