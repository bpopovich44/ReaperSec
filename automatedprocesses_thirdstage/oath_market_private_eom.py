...
def connect():
    ...
    db = "mysql_eom"
    report_type = "market_private_eom"
    p_name = sys.argv[1]
            ...
            for report in report_book[report_type][p_name]:
                ...
                if len(result) >= 1:
                    if db_updated == False:
                        ...
                        sql = "DROP TABLE IF EXISTS " + p_name + "_market_private_EOM"
                        cursor.execute(sql)

                        sql = "CREATE TABLE " + p_name + "_market_private_EOM (date varchar(25), inventory_source varchar(255), \
                ...
                print(str(todaysdate) + "  Running " + p_name + "_market_private_eom id_" + str(p_id) + " with report # " + str(report))
	            ...
	            sql = """INSERT INTO """ + p_name + """_market_private_EOM VALUES ("%s", "%s", "%s", "%s", "%s", "%s", \
                            "%s", "%s", "%s", "%s", "%s", "%s", "%s,", "%s")""" % (date, inventory_source, geo_country, \
                            buyer_organization, ad_opportunities, ad_attempts, ad_impressions, \
			    ad_revenue, media_spend, ecpm, completed_views, clicks, platform, market_id)
	            cursor.execute(sql)
