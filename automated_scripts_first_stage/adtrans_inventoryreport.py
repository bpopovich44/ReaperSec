...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS adtrans_inventoryreport"
            cursor.execute(sql)

            sql = "CREATE TABLE adtrans_inventoryreport (date varchar(50), inventory_source varchar(255), geo_country varchar(50), \
		...
		ad_revenue decimal(15, 5), ecpm decimal(15, 5), platform int)"
            ...
            result = aol_api_R.run_existing_report(logintoken, "169697")
            #print(result)
        	...
        	sql = """INSERT INTO adtrans_inventoryreport VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s")""" % \
