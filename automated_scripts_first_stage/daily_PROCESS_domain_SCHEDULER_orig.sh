...
process_domain_data(){
	echo ${path_sbin}"${platform}_domainreport_v2.py" 
	for report in "${reports[@]}"
	do
     		echo ${path_sbin}"${platform}_InventorySources_v2_${report}.py"
	done
	}
...
process_data(){
	...
	[[ -n $check_db ]] && echo ${path_pan}-file="${path_ktr}${platform}_domainreport_v2.ktr" || return 1
