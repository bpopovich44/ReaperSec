            ...
            while True:
                ...
                if len(info) >= 1:
                    if db_updated == False:
                        ...
                        sql = "CREATE TABLE springserve_InventorySources_v2(rownum INT NOT NULL AUTO_INCREMENT PRIMARY KEY, \
                            date varchar(25), inventory_source varchar(255), media varchar(255), ad_opportunities bigint, \
        	                ad_attempts bigint, ad_impressions bigint, ad_revenue decimal(15,5), clicks int)"
                        cursor.execute(sql)

                        db_updated = True

                print(str(todaytime) + "  Running springserve_InventorySources_v2.  Page # " + str(page) + " Count " + str(len(info)))
                ...
                for x in info:
                    ...
                    sql = """INSERT INTO springserve_InventorySources_v2 VALUES ("%s", "%s", "%s", "%s", "%s", \
                            "%s", "%s", "%s", "%s")""" % (rownum, date, inventory_source, media, ad_opportunities, \
                            ad_attempts, ad_impressions, ad_revenue, clicks)
                    cursor.execute(sql)
