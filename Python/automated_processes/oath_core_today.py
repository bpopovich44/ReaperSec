    ...
    try:
        ...
        if conn.is_connected():
            ...
            for report in report_book[report_type][p_name]:
                ...
                #print(row_count_value)
                
                if int(row_count_value) >= 1:
                    if db_updated == False:

                        sql = "DROP TABLE IF EXISTS " + p_name + "_core_today"
                        cursor.execute(sql)

                        sql = "CREATE TABLE " + p_name + "_core_today (date varchar(50), hour int, inventory_source varchar(255), ad_opportunities bigint, \
                            market_opportunities bigint, ad_attempts bigint, ad_impressions bigint, ad_errors bigint, \
                            ad_revenue decimal(15, 5), aol_cost decimal(15, 5), epiphany_gross_revenue decimal(15, 5), \
                            " + p_name + "_revenue decimal(15, 5), total_clicks int, iab_viewability_measurable_ad_impressions bigint, \
                ...
                print(str(todaysdate) + "  Running " + p_name + "_core_today report # " + str(report))
	            ...
	            #print(list)
                    ...
                    if p_name == 'dna':
     	            ...
     	            sql = """INSERT INTO """ + p_name + """_core_today VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", \
                            "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s")""" % (date, hour, inventory_source, ad_opportunities, \
