            ...
            for report in report_book[report_type][p_name]:
                ...
                if len(result) >= 1:
                    if db_updated == False:
                        ...
                        sql = "DROP TABLE IF EXISTS " + p_name + "_market_public"
                        cursor.execute(sql)

                        sql = "CREATE TABLE " + p_name + "_market_public (date varchar(25), inventory_source varchar(255), \
                            geo_country varchar(50), ad_opportunities varchar(2), ad_attempts bigint, \
                            ad_impressions bigint, ad_revenue decimal(15, 5), ecpm decimal(6, 4), \
                            media_spend decimal(15, 5), completed_views int, clicks int, platform int, market_id int)"
                        cursor.execute(sql)

                        db_updated = True

                print(str(todaysdate) + "  Running " + p_name + "_market_public id " + str(p_id) + " with report # " + str(report))
	            ...
	            sql = """INSERT INTO """ + p_name + """_market_public VALUES ("%s", "%s", "%s", "%s", "%s", "%s", \
                            "%s", "%s", "%s", "%s", "%s", "%s", "%s")""" % (date, inventory_source, geo_country, \
                            ad_opportunities, ad_attempts, ad_impressions, ad_revenue, ecpm, media_spend, \
                            completed_views, clicks, platform, market_id)
        	    cursor.execute(sql)
