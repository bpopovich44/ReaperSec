...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS dc_market_public_EOM"
            cursor.execute(sql)

            sql = "CREATE TABLE dc_market_public_EOM (date varchar(25), inventory_source varchar(255), geo_country varchar(50), \
                    ad_opportunities varchar(2), ad_attempts bigint, ad_impressions bigint, \
            ...
            result = aol_api.run_existing_report(logintoken, "193894")
            #print(result)
	        ...
	        sql = """INSERT INTO dc_market_public_EOM VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", \
			"%s", "%s", "%s")""" % (date, inventory_source, geo_country, ad_opportunities, \
