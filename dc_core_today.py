...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            # Create Table:
            sql = "DROP TABLE IF EXISTS dc_core_today"
            cursor.execute(sql)

            sql = "CREATE TABLE dc_core_today (hour int, inventory_source varchar(255), ad_opportunities bigint, \
            ...
            result = aol_api.run_existing_report(logintoken, "143817")
            #print(result)
...
#	        print(list)
        	...
        	sql = """INSERT INTO dc_core_today VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s"*.20, "%s"*.56, \
