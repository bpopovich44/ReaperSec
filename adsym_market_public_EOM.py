...
def connect():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            sql = "DROP TABLE IF EXISTS adsym_market_public_EOM"
            cursor.execute(sql)

            sql = "CREATE TABLE adsym_market_public_EOM (date varchar(25), inventory_source varchar(255), geo_country varchar(50), \
            ...
            result = aol_api_R.run_existing_report(logintoken, "193897")
            #print(result)
	        ...
	        sql = """INSERT INTO adsym_market_public_EOM VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", \
